...
# Phase B bleeder caps — limit position to ±N for products that bled
# spread/vol (informed-flow adverse selection) on the v1 backtest.
PROD_CAP = {
...
class Trader:
    def run(self, state: TradingState):
        ...
        # UV_VISOR_AMBER sine fair-value deviation
        sine_skew = {}
        if False and "UV_VISOR_AMBER" in mids:
            sine_dev = UV_AMBER_A * sin(UV_AMBER_OMEGA * ts + UV_AMBER_PHI)
            mid_amb = mids["UV_VISOR_AMBER"]
            # We treat sine as the high-frequency deviation expectation; if mid
            # is above (mid - sine_dev) we expect retrace down → negative skew.
            # Compare mid to a slow trailing baseline (use mid itself; the
            # trend term cancels because sine_dev oscillates symmetrically).
            sine_skew["UV_VISOR_AMBER"] = max(-1.5, min(1.5, -sine_dev / 80.0))
