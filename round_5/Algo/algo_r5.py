...
# Per-product cap below the global limit (bleeders found by ablation).
PROD_CAP = {
...
# Dedicated ROBOT_DISHES handler params.
DISHES_AR1_COEF = -0.232
DISHES_AR1_ALPHA = 0.0
DISHES_LOG_CLIP = 10
DISHES_INV_BETA = 0.6
DISHES_USE_AR1 = False
DISHES_USE_LOG = True

# 4 log-space pairs feeding the dedicated ROBOT_DISHES tilt.
DISHES_LOG_PAIRS = [
...
def _pair_skews_all(mids: Dict[str, float]) -> Dict[str, float]:
    # ROBOT_DISHES is exempt: its tilt comes only from the dedicated handler.
    out: Dict[str, float] = {}
...
def _dishes_log_pair_skew(mids: Dict[str, float]) -> float:
    """Sum log-pair tilts for ROBOT_DISHES, in price-space dollars.
    For each pair (i, j, β, α), residual r = log(p_i) − β log(p_j) − α.
    Tilt is −r when DISHES is the i-side, and sign(β)·r when on the j-side."""
    ...
    for i, j, beta, alpha in DISHES_LOG_PAIRS:
        if i not in mids or j not in mids:
            continue
        if i == "ROBOT_DISHES":
            log_resid = log_p_d - beta * math.log(mids[j]) - alpha
            tilt_log = -log_resid
        else:
            log_resid = math.log(mids[i]) - beta * log_p_d - alpha
            tilt_log = (1.0 if beta >= 0 else -1.0) * log_resid
...
def _dishes_dedicated_skew(mids: Dict[str, float], td: dict) -> float:
    ...
    if DISHES_USE_AR1:
        prev = td.get("pmd")
        if prev is not None:
            skew += -DISHES_AR1_COEF * (cur - prev) * DISHES_AR1_ALPHA
        td["pmd"] = cur
    if DISHES_USE_LOG:
